...
For the purposes of this training session, we have provisioned a single {ocp} cluster, with {rhoai} deployed on it. In addition we have set up Amazon Bedrock (Amazon Nova Pro v1) and Bedrock Knowledge Base that we will connect to via their respective APIs. 
...
* The OpenShift Console URL for our shared environment:
** {ocp_console_url}[{ocp_console_url},window=_blank]

* Click on the `Cognito` button:
+
[.bordershadow]
...
+
[.bordershadow]
image::02/02-01-ocp-projects-page.png[]

* Click on the application switcher in the top right corner (9 dots) and click on Red Hat OpenShift AI:
+
[.bordershadow]
image::02/02-01-app-switch.png[]
...
* If you are asked to log in, press the button and select the Cognito option
